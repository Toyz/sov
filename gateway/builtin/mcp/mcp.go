...
// exposes the gateway's registered routers as MCP tools — so any MCP client
// (Claude, Cursor, Copilot) can call your sov methods as tools. It is the
// MCP arm of PEMM: the SAME router struct serves RPC, mesh, AND MCP.
//
// tools/list is reflected from the engine's Describe() (tool name, JSON
// Schema from the params, description from the sov doc metadata); hard/soft
// hidden methods are excluded. tools/call re-dispatches through gw.Handle,
// so the FULL request chain — auth, authz, and the declarative perm
// (HELL-280) — gates an MCP call exactly as it gates the same method over
// /rpc. There is no separate MCP capability model: MCP rides the perm system.
//
...
func (p *Plugin) Doc() string {
	return "Model Context Protocol server at " + p.path + " — exposes registered routers as MCP tools; tools/call rides the same auth/authz/perm as /rpc."
}
